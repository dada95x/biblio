...
int get_list_of_books(struct Book books[])
...
return : the number of books borrowed */
{
	...
	while(i<len){
		for(i; str[i]!=';'&&i<len; i++) //as long as we don't meet the caracter ";"
...
int book_already_in_library(char* title, char* author, struct Book books[])
