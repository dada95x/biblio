...
#define KEY "CYTECHLIBRARY" //the Vigenere key for coding the password in the file
...
#define MAX_DIGITS_ID 7 // the maximum of digits allowed for a book id
...
#define MAX_GENRE 30 //the max amount possible of genres
#define MAX_USERS 200 // the maximum amount of users of the application
#define MAX_BOOKS 1000 //the maximum amount of books the library can hold
...
#endif
#if defined(_WIN32) //macro to write on the files according to the difference of new lines in Windows and Linux
    #define NEWLINE fputs("\n", f); // for Windows, we put a new line
#elif defined(__linux__)
	#define NEWLINE  // for Linux, we don't
#endif
struct User{
...
int get_list_of_books(struct Book[]);
...
int book_already_in_library(char*, char*, struct Book[]);
