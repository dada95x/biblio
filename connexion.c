...
return : an int which will tell if the connexion was successful (0) or not (-1) */
{
	...
	int nb_tries = 1;
	while(strcmp(password_entered,password_decoded)!=0 ) //checks if the decoded password matches the entered password
	{
		nb_tries++; 
		if(nb_tries>MAX_TRIES_PASSWORD) // if the number of tries exceed the amount allowed
			break;
